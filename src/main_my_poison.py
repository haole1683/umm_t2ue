...
from .data import load_my_poison
...
def worker(rank, options, logger):
    ...
    data = load_my_poison(options, processor)

    optimizer = None
    scheduler = None
    if(data["train"] is not None):    
        # All model parameter need to be optimized !!    
...
if(__name__ == "__main__"):    
    ...
    # model, processor = load_model(name = options.model_name, pretrained = options.pretrained)
    # print(processor)
    # all_data = load_my_poison(options, processor)
    # train_data = all_data["train"]
    # print(train_data[0])
    
    options.train_data = os.path.expanduser(options.train_data)
    options.eval_test_data_dir = os.path.expanduser(options.eval_test_data_dir)
    ...
    if(ngpus == 0 or options.device == "cpu"):
        ...
        if(ngpus == 1 or not options.distributed):
            ...
            options.distributed = False
            # worker(0, options, logger)
            # cProfile.run('worker(0, options, logger)')
            cProfile.run('worker(0, options, logger)', filename='performance_analysis.prof')
