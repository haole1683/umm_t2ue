...
# 微调模型 + 噪音，在原始图像可见的情况下进行优化。优化采用PGD，依次优化模型、图像（PGD）和文本。
...
def poison(epoch, model, data, optimizer, scheduler, scaler, options,processor):
    ...
    elif dataloader.num_samples<=145000:
        data_samples_num = 145000
    elif dataloader.num_samples<=566435:
        data_samples_num = 566435
        
    print(data_samples_num)
    ...
    step_size = ((1 / 255) / var)
    epsilon = ((options.epsilon / 255) / var)
    num_steps = 8
        ...
        for _, (batch,index) in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            if text_time<2:
                ...
                with torch.no_grad():
                    ...
                    pixel_values = clamp(pixel_values + noise[index], lower_limit, upper_limit)
                    for i, _ in enumerate(batch["pixel_values"]):
                ...
                candidates = attacks.hotflip_attack(grad, embedding_weight,
                                                    increase_loss=False, num_candidates=6,
