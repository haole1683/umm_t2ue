import os
# os.environ['PYTORCH_CUDA_ALLOC_CONF'] = '0'  # 设置为0以禁用内存分片，这里不能设置，会报错
...
from .poison_train import poison
from .train import train
from .evaluate import evaluate, Finetune
from .data import poison_load as load_data
...
def worker(rank, options, logger):
    ...
    if (options.device == "cpu"):
        ...
        the_device = options.device_ids[options.rank] if options.distributed else options.device_id
        print(f"Setting{ the_device}")
    ...
    if (data["train"] is not None):
        ...
        for epoch in range(start_epoch + 1, options.epochs + 1):
            ...
            start = time.time()
            noise,trigger_token_ids = poison(epoch, model, data, optimizer, scheduler, scaler, options,processor)
