...
class ImageCaptionDataset_Poisoned_WithoutTokens(Dataset):
...
class ImageCaptionDataset_percent(Dataset):
    def __init__(self, path, image_key, caption_key, delimiter, processor, noise, token,options, inmodal=False, defense=False, crop_size=150,):
        logging.debug(f"Loading aligned data from {path}")
        ...
        # self.captions_text = [self.captions_text[i] for i in sample_indices]
    ...
    def __getitem__(self, idx):
        ...
        if (self.inmodal):
            ...
            item["input_ids"] = self.captions["input_ids"][idx]
            if self.token is not None and idx in self.sample_indices:
                item["input_ids"][1:1 + self.num_trigger_tokens] = self.token[idx]
            item["attention_mask"] = self.captions["attention_mask"][idx]
            item["pixel_values"] = self.processor.process_image(image) 
            if self.noise is not None and idx in self.sample_indices:
                # print(item["pixel_values"].shape)
                # print(self.noise[idx].shape)
                item["pixel_values"] += self.noise[idx]
...
def get_my_poison_train_dataloader(options, processor):
    np.random.seed(12)
    torch.manual_seed(12)
    torch.cuda.manual_seed(12)
    torch.cuda.manual_seed_all(12)
    ...
    batch_size = options.batch_size
    my_noise_path = options.noise_path
    noise = torch.load(my_noise_path)
    dataset = ImageCaptionDataset_PoisonImage(path, image_key = options.image_key, caption_key = options.caption_key, delimiter = options.delimiter, processor = processor,noise=noise, inmodal = options.inmodal)
    # print(dataset[0])
    # print("len is " , len(dataset))
    sampler = DistributedSampler(dataset) if(options.distributed) else None
...
def get_train_dataloader_percent(options, processor):
    path = options.train_data
    if (path is None): return None
...
def load_my_poison(options, processor):
    data = {}
    
    data["train"] = get_my_poison_train_dataloader(options, processor)
    data["validation"] = get_validation_dataloader(options, processor)
    data["eval_test"] = get_eval_test_dataloader(options, processor)
    data["eval_train"] = get_eval_train_dataloader(options, processor)
...
def load_percent(options, processor):
    data = {}
    
    data["train"] = get_train_dataloader_percent(options, processor)
    data["validation"] = get_validation_dataloader(options, processor)
    data["eval_test"] = get_eval_test_dataloader(options, processor)
    data["eval_train"] = get_eval_train_dataloader(options, processor)
...
def get_poison_test_dataloader(options, processor):
    ...
    try:
        noise_path = os.path.join(options.save_path, "noise.pt")
        print("Loading noise from", noise_path)
        noise = torch.load(os.path.join(options.save_path, "noise.pt"))
    except:
        noise = None
    try:
...
def get_poison_test_dataloader_without_tokens(options, processor):
    path = options.train_data
    if (path is None): return None
    ...
    dataset = ImageCaptionDataset_Poisoned_WithoutTokens(path, image_key=options.image_key, caption_key=options.caption_key,
                                  delimiter=options.delimiter, processor=processor, inmodal=options.inmodal,noise=noise,token=token,options=options)
...
def get_poison_tsne_dataloader(options, processor):
    path = options.train_data
    if (path is None): return None
    ...
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=options.num_workers,
                            pin_memory=True, sampler=sampler, drop_last=True)
    dataloader.num_samples = len(dataloader) * batch_size
    dataloader.num_batches = len(dataloader)
...
def poison_tsne_load(options, processor):
    data = {}
...
def poison_test_load_without_tokens(options, processor):
    data = {}

    data["train"] = get_poison_test_dataloader_without_tokens(options, processor)
    data["validation"] = get_validation_dataloader(options, processor)
    data["eval_test"] = get_eval_test_dataloader(options, processor)
    data["eval_train"] = get_eval_train_dataloader(options, processor)
